...
<PRIORITY_TASK>
For each origin state (i.e., each state provided in the input except {region_id}), determine the inbound traffic allocation into {region_id} for the next 8 weeks.
For each origin state, total inbound Flow over 8 weeks must stay constant (equal to the ground truth); you may adjust weekly proportions.

**Constraints:**
- Output 8 fractions [f1, f2, f3, f4, f5, f6, f7, f8], each in [0,1].
- Week t inbound flow = Flow * ft.
- Low ft = stricter control; high ft = relaxed control.
- f1+f2+f3+f4+f5+f6+f7+f8 must equal exactly 1 (3-4 decimals).
...
5. **Validation** that the 8 fractions sum to 1.
...
## The 8 fractions must sum to exactly 1.  
