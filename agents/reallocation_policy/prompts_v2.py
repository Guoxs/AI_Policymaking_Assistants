...
## Your task
For each neighboring state of **{region_id}**, determine the inbound traffic allocation across the next 2 weeks.  
Since we do not want to change the total amount of travel in the following two weeks (Flow), you could adjust the allocation for each week here.
**Constraints and Instructions:**
- Output a vector of 2 numeric fractions [f1, f2], each in [0,1].  
- f1, f2 correspond to Week 1 and Week 2 respectively, and the sum of flow at week 1 is Flow * f1, week 2 is Flow * f2.  
...
- The 2 fractions (f1+f2) must sum exactly to 1 (use 3-4 decimal precision if needed),  Flow * (f1 + f2) = Flow.
Objective: minimize cumulative confirmed and deceased cases in the current state

## Rules:
- Fractions (f1,f2) must be numeric with exactly 2 entries per origin; sum to 1 (use 3-4 decimals if needed).
...
{{
  "state_name": {{
      "fractions": [f1, f2],    // 2 numbers in [0,1], sum exactly to 1
