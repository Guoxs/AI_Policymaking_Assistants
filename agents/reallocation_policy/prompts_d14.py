...
## Your task
For each neighboring state of **{region_id}**, determine the inbound traffic allocation across the next 14 days.  
Since we do not want to change the total amount of travel in the following 14 days (Flow), you could adjust the allocation for each day here.
**Constraints and Instructions:**
- Output a vector of 14 numeric fractions [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14], each in [0,1].  
- f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14 correspond to Day 1, Day 2, Day 3, Day 4, Day 5, Day 6, Day 7, Day 8, Day 9, Day 10, Day 11, Day 12, Day 13, Day 14 respectively, and the sum of flow at day i is Flow * fi.  
- a low fi means stricter traffic control on day i, while a high fi means relaxed control.
- The 14 fractions (f1+f2+f3+f4+f5+f6+f7+f8+f9+f10+f11+f12+f13+f14) must sum exactly to 1 (use 3-4 decimal precision if needed),  Flow * (f1 + f2 + f3 + f4 + f5 + f6 + f7 + f8 + f9 + f10 + f11 + f12 + f13 + f14) = Flow.
Objective: minimize cumulative confirmed and deceased cases in the current state

## Rules:
- Fractions (f1,f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14) must be numeric with exactly 2 entries per origin; sum to 1 (use 3-4 decimals if needed).
...
{{
  "state_name": {{
      "fractions": [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14],    // 14 numbers in [0,1], sum exactly to 1
