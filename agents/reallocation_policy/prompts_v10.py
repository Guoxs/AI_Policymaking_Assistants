...
<PRIORITY_TASK>
For each origin state (i.e., each state provided in the input except {region_id}), determine the inbound traffic allocation into {region_id} for the next 10 weeks.
For each origin state, total inbound Flow over 10 weeks must stay constant (equal to the ground truth); you may adjust weekly proportions.

**Constraints:**
- Output 10 fractions [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10], each in [0,1].
- Week t inbound flow = Flow * ft.
- Low ft = stricter control; high ft = relaxed control.
- f1+f2+f3+f4+f5+f6+f7+f8+f9+f10 must equal exactly 1 (3-4 decimals).
...
5. **Validation** that the 10 fractions sum to 1.
...
## The 10 fractions must sum to exactly 1.  
## Each key "state_name" refers only to an origin state (all states except {region_id}).
```json
{{
  "state_name": {{
      "fractions": [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10],
