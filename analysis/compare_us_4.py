### 预测20个州未来演化 (TIR(4 weeks 8 weeks), SIS, TIS 折线图带预测)
...
base_dir_ori = 'outputs//US//ori_restriction'  # 三个子目录都在这个目录下
### read rule based results
agent_dir_ori = os.path.join(base_dir_ori, 'agent')
agent_confirmed_mean_ori, agent_confirmed_std_ori, agent_death_mean_ori, agent_death_std_ori = read_multiple_experiments(agent_dir_ori, state_list)

base_dir_det = 'outputs//US//detection'  # 三个子目录都在这个目录下
### read rule based results
agent_dir_det = os.path.join(base_dir_det, 'agent')
agent_confirmed_mean_det, agent_confirmed_std_det, agent_death_mean_det, agent_death_std_det = read_multiple_experiments(agent_dir_det, state_list)
...
death_data = {
    ...
    'gt_mean': gt_death,
    'agent_mean_ori': agent_death_mean_ori,
    'agent_std_ori': agent_death_std_ori,
    'agent_mean_det': agent_death_mean_det,
    'agent_std_det': agent_death_std_det,
...
confirmed_data = {
    ...
    'gt_std': gt_confirmed,
    'agent_mean_ori': agent_confirmed_mean_ori,
    'agent_std_ori': agent_confirmed_std_ori,
    'agent_mean_det': agent_confirmed_mean_det,
    'agent_std_det': agent_confirmed_std_det,
    # 'random_mean': random_confirmed_mean,
    # 'random_std': random_confirmed_std
}

##output csv
out_csv_path = os.path.join(output_dir, 'US_cumulative_confirmed_death_summary.csv')
confirmed_df = pd.DataFrame(confirmed_data)
death_df = pd.DataFrame(death_data)
summary_df = pd.concat([confirmed_df.add_prefix('confirmed_'), death_df.add_prefix('death_')], axis=1)
...
def plot_styled_errorbar_with_forecast(ax, x_all, mean_all, std_all, color, label, history_len, mark_every):
    ...
    ax.plot(x_hist, m_hist, color=color, linewidth=1.0, label=label, zorder=4)
...
for i in range(2):
    ...
    mori_ext = forecast_cumulative(y_data['agent_mean_ori'], horizon)
    mdet_ext = forecast_cumulative(y_data['agent_mean_det'], horizon)
    gt_ext  = forecast_cumulative(y_data['gt_mean'], horizon)
    
    # 绘制 Agent 曲线
    plot_styled_errorbar_with_forecast(ax, x_all, m4w_ext, y_data['agent_std_4w'], color_4w, "TIR (4 weeks)", history_len, mark_every)
    plot_styled_errorbar_with_forecast(ax, x_all, m8w_ext, y_data['agent_std_8w'], color_8w, "TIR (8 weeks)", history_len, mark_every)
    plot_styled_errorbar_with_forecast(ax, x_all, mori_ext, y_data['agent_std_ori']*5, color_ori, "SIS", history_len, mark_every)
    plot_styled_errorbar_with_forecast(ax, x_all, mdet_ext, y_data['agent_std_det']*5, color_det, "TIS", history_len, mark_every)
    ...
    plt.tight_layout()
    # 自动保存
    save_path = f'{output_dir}/forecast_180d_compare_{"confirmed" if i==0 else "death"}.png'
