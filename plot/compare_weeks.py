...
for state in state_list:
    ...
    cadences = [
        ...
        ('6 weeks//gpt-3.5//metrics',  '6 weeks',  '#D55E00', 'Agent (6 weeks)'),   # vermillion
    ...
    y_labels    = ['IR (per 100k)', 'ACR (per 100k)', 'DR (per 100k)']   #per 100k
    ...
    for index in range(len(y_labels)):
        ...
        for subdir, suffix, color, label in cadences:
            ...
            # 画线 + 阴影
            plt.plot(dates, mean_s, label=label, color=color, linewidth=2.0, marker='o',
                     markersize=10,
        ...
        plt.ylabel(y_labels[index], fontsize=30)
        plt.xlabel('Date', fontsize=30)
        if state == 'virginia' and index == 2:
            plt.ylim(0,1)
        ...
        plt.grid(alpha=0.3)
        plt.tight_layout()
        #plt.savefig(f"figures/5 states/weeks compare/{state}_{y_labels[index]}.png")
        plt.savefig(f"figures/5 states/SI_week/{state}_{y_labels[index]}.png")
        plt.close()
