#### 时间序列图 绘制政策效果对比（不同政策类型下的 LLM Agent, 折线图）
...
agent_confirmed_mean_2, agent_confirmed_std_2, agent_death_mean_2, agent_death_std_2 = read_multiple_experiments(agent_dir_2, state_list)

agent_confirmed_mean_3, agent_confirmed_std_3, agent_death_mean_3, agent_death_std_3 = read_multiple_experiments(agent_dir_3, state_list)
...
death_data = {
    ...
    'agent_mean_2': agent_death_mean_2,
    'agent_std_2': agent_death_std_2,
    'agent_mean_3': agent_death_mean_3,
    'agent_std_3': agent_death_std_3,
    'gt_mean': gt_death,
    'gt_std': gt_death,
...
confirmed_data = {
    ...
    'agent_mean_2': agent_confirmed_mean_2,
    'agent_std_2': agent_confirmed_std_2,
    'agent_mean_3': agent_confirmed_mean_3,
    'agent_std_3': agent_confirmed_std_3,
...
for i in range(2):
    ...
    ax.plot(
        x, y_data['agent_mean'],
        color=colors[0], linewidth=2.4,
        linestyle='-',
        marker='o', markersize=8, markevery=mark_every,
        markerfacecolor='white', markeredgecolor=colors[0], markeredgewidth=1.0,
        label=type_name[0],
        zorder=4
    )
    ax.fill_between(
    ...
    # rule_color = "#D55E00"
    ax.plot(
        x, y_data['agent_mean_2'],
        color=colors[1], linewidth=2.0,
        linestyle=DASH_RULE,
        marker='^', markersize=8, markevery=mark_every,
        markerfacecolor='white', markeredgecolor=colors[1], markeredgewidth=1.0,
        label=type_name[1],
        zorder=3
    )
    ax.fill_between(
        x,
        np.clip(y_data['agent_mean_2'] - y_data['agent_std_2'], 0, None),
        y_data['agent_mean_2'] + y_data['agent_std_2'],
        alpha=0.14,
        zorder=1, color=colors[1]
    )

    ax.plot(
        x, y_data['agent_mean_3'],
        color=colors[2], linewidth=2.0,
        linestyle=DASH_RULE,
        marker='^', markersize=8, markevery=mark_every,
        markerfacecolor='white', markeredgecolor=colors[2], markeredgewidth=1.0,
        label=type_name[2],
        zorder=3
    )
    ax.fill_between(
        x,
        np.clip(y_data['agent_mean_3'] - y_data['agent_std_3'], 0, None),
        y_data['agent_mean_3'] + y_data['agent_std_3'],
        alpha=0.14, color=colors[2],
        zorder=1
    )
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/compare_policy{"confirmed" if i==0 else "death"}.png', dpi=300)
    plt.show()