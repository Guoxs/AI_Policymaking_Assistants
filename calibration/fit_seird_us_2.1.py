...
def objective(params):
    ...
    sta_index = sta_index[(sta_index['fips'] != 2) & (sta_index['fips'] != 15)]   #去除阿拉斯加和夏威夷
    state_name = CONFIG['region_names']
    sta_index = sta_index[sta_index['state_name'].str.lower().isin(state_name)]
    init_state['region_ids'] = state_name
    ...
    init_state = load_mobility_data(init_state, CONFIG['mobility_path_us'],len(init_state['region_ids']))
    
    # load population data
    init_state = load_population_data(init_state, CONFIG['dataset_path_us'], CONFIG['path_suffex_us'])
...
def compute_loss_2(list_df):
    ...
    for i, df in enumerate(list_df):
        ...
        deaths_error = (df['D_pred'] - df['D_gt'])**2
        has_R =  not np.all(df['R_gt'] == 0) and not np.any(np.isnan(df['R_gt']))
        confirmed_loss = (confirmed_error / (df['Q_gt'].mean()*df['Q_gt'].mean() + 1e-6)).mean()
        deaths_loss = (deaths_error / (df['D_gt'].mean()*df['D_gt'].mean() + 1e-6)).mean()
        if has_R:
            recovered_error = (df['R_pred'] - df['R_gt'])**2
            recovered_loss = (recovered_error / (df['R_gt'].mean()*df['R_gt'].mean() + 1e-6)).mean()
            region_loss = confirmed_loss + deaths_loss + recovered_loss
            region_loss = region_loss/3
        else:
            region_loss = confirmed_loss + deaths_loss
...
def compute_loss_4(list_df):
    loss = 0.0
    state_loss = {}
    for i, df in enumerate(list_df):
        confirmed_error = abs(df['Q_pred'] - df['Q_gt'])
        deaths_error = abs(df['D_pred'] - df['D_gt'])
        has_R =  not np.all(df['R_gt'] == 0) and not np.any(np.isnan(df['R_gt']))
        confirmed_loss = (confirmed_error / (df['Q_gt'].max() + 1e-6)).mean()
        deaths_loss = (deaths_error / (df['D_gt'].max() + 1e-6)).mean()
        if has_R:
            recovered_error = abs(df['R_pred'] - df['R_gt'])
            recovered_loss = (recovered_error / (df['R_gt'].max() + 1e-6)).mean()
            region_loss = confirmed_loss + deaths_loss + recovered_loss
            region_loss = region_loss/3
        else:
            region_loss = confirmed_loss + deaths_loss
...
def optuna_objective(trial):
    ...
    path =  os.path.join(root_path, 'datasets', 'US','seird_parameters_step1.csv')  #v3_0.185 best for 21 states
    ...
    for i in range(num_regions):
        ...
        #beta = trial.suggest_float(f"beta_{i}", 0.02, 2.0)     # the bound for beta
...
if __name__ == "__main__":
    ...
    sampler=TPESampler(n_startup_trials = 100, multivariate=False, group=False),  # 使用TPE算法; n start trail 100: 0.185; multivariate false: 
    ...
    print("\n 最优目标函数值 (loss):x", study.best_value)
    ...
    root_path = pathlib.Path(__file__).parent.resolve()
    path =  os.path.join(root_path, 'datasets','US','seird_parameters_step1.csv')
    df_params = pd.read_csv(path)
    df_params = df_params[df_params['State'].isin(CONFIG['region_names'])]
    flat_best = list(best_params.values())
    for i, state in enumerate(CONFIG['region_names']):
        scale = best_params[f"beta_scale_{i}"]
        df_params.loc[df_params['State'] == state, 'beta'] *= scale
    # print("拟合完成，最优参数：")
    print(df_params)

    #df_params.to_csv('D:\MyDownload\Code\OD-COVID\datasets\seird_parameters.csv', index=False)
    df_params.to_csv(os.path.join(root_path, 'datasets','US','seird_parameters_step2.csv'), index=False)
